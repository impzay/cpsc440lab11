...
int main(void)
{
	...
	int xOff = 0;
	int yOff = 0;
	if(MapLoad("isaiahmap.fmp", 1))
		return -5;

	event_queue = al_create_event_queue();
	timer = al_create_timer(1.0 / 60);
	endtimer = al_create_timer(10.0);

	al_register_event_source(event_queue, al_get_timer_event_source(timer));
	al_register_event_source(event_queue, al_get_timer_event_source(endtimer));
	...
	while(!done)
	{
		...
		{	
			if (ev.timer.source == endtimer) {
				done = true;
				cout << "ended game because of block timer.\n";
			}
			render = true;
			if(keys[UP])
				;
			...
			if (player.CollisionEndBlock()) {
				cout << "Hit an End Block\nClosing the game in 10 seconds.\n";
				al_start_timer(endtimer);
			}
			render = true;
